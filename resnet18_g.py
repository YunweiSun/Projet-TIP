...
class Bottleneck(nn.Module):
    ...
    """
    expansion = 1   # 残差块第3个卷积层的通道膨胀倍率
    def __init__(self, in_channel, out_channel, stride=1, downsample=None):
        super(Bottleneck, self).__init__()

        self.conv1 = nn.Conv2d(in_channels=in_channel, out_channels=out_channel, kernel_size=3, stride=1, bias=False, padding=1)   # H,W不变。C: in_channel -> out_channel
...
class ResNet(nn.Module):
    ...
    def forward(self, x):
        ...
        # out = torch.unsqueeze(prob.argmax(dim=1), dim=1)

        return prob # 输出概率
        

def resnet18_g(num_classes=2):
    return ResNet(block=Bottleneck, block_num=[2, 2, 2, 2], num_classes=num_classes)



if __name__ == '__main__':
    input = torch.randn(1, 3, 224, 224)  # B C H W
    ResNet50 = resnet18_g(1000)
